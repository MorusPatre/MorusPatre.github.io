...
document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    function updateSidebarMarqueeHighlight(galleryRect, visibleMarqueeRect) {
        ...
        const leftEdgeRight = Math.min(zone.right, zone.left + MARQUEE_WINDOW_EDGE_HIGHLIGHT_WIDTH + MARQUEE_GLOW_OUTSET);
        const leftEdgeTop = Math.max(marqueeViewportRect.top - MARQUEE_GLOW_OUTSET, zone.top);
        const leftEdgeBottom = Math.min(marqueeBottom + MARQUEE_GLOW_OUTSET, zone.bottom);
        ...
        const bottomEdgeTop = Math.max(zone.top, zone.bottom - MARQUEE_BOTTOM_EDGE_HIGHLIGHT_HEIGHT - MARQUEE_GLOW_OUTSET);
        const bottomEdgeLeft = Math.max(marqueeViewportRect.left - MARQUEE_GLOW_OUTSET, zone.left);
        const bottomEdgeRight = Math.min(marqueeRight + MARQUEE_GLOW_OUTSET, zone.right);
    ...
    function updateMarqueeAndSelection(clientX, clientY, isModifier) {
        ...
        const visibleBottom = shouldOverscanBottom
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getSidebarBounds() {
        ...
        const min = getNumber(styles.getPropertyValue('--sidebar-min-width'), 176);
        const max = getNumber(styles.getPropertyValue('--sidebar-max-width'), 420);
        const viewportMax = Math.max(min, Math.min(max, window.innerWidth - 320));
    ...
    function setSidebarCollapsed(collapsed, restoreWidth = null) {
        ...
        setSidebarWidth(restoreWidth !== null ? restoreWidth : lastExpandedWidth || currentSidebarWidth || bounds.min, bounds);
    ...
    function cancelQueuedSidebarWidth() {
        if (resizeFrame !== null) {
            cancelAnimationFrame(resizeFrame);
            resizeFrame = null;
        }
    ...
    handle.addEventListener('pointerdown', (event) => {
        ...
        function onPointerMove(moveEvent) {
            ...
            if (nextWidth < dragCollapseThreshold) {
                cancelQueuedSidebarWidth();
                setSidebarCollapsed(true);
                return;
            }

            if (isSidebarCollapsed()) {
                setSidebarCollapsed(false, nextWidth);
            }
